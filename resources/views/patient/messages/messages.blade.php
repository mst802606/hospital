																...
																@foreach ($messagesdata['messages'] as $message)
																				<a href="{{ route('patient.messages.show', ['message' => $message->id]) }}">
																								<div>
																												...
																												@else
																																<h5><small><span class="badge badge-danger">Waiting response</span></small></h5>
																												@endif
																				...
																				<section class="message message-container overflow-auto" id="chat-section">
																								<div class="parent">
																												...
																												<div class="container">
																																...
																																<p><strong>{{ 'Created by ' . $message->patient->user->username }}</strong></p>
																																<p id="messages_title">{{ 'Title: ' . $message->title }}</p>
																																...
																																@foreach ((array) json_decode($message->replies) as $key => $reply)
																																				@if ($reply->key == 'doctor')
																																								<div class="container darker">
																																												<div class="col-md-3 col-xl-2"><img class="logo w-25"
																																																				...
																																																				style="width: 100%" /></div>
																																												<p><strong>{{ $reply->key . ' ' . $message->doctor->user->username }}</strong>
																																												</p>
																																												<p>{{ $reply->message }}</p>
																																												<span
																																				...
																																				@else
																																								<div class="container">
																																												<div class="col-md-3 col-xl-2"><img class="logo w-25"
																																																				src="{{ url('images/logo.png') }}" alt="Avatar" /></div>
																																												<p><strong>{{ 'You ' . $message->patient->user->username }}</strong></p>
																																												<p>{{ $reply->message }}</p>
																																												<span
																																																class="time-right">{{ date('H:i d-m-Y', strtotime($reply->time)) }}</span>
																																								</div>
																												...
																												<div class="child" id="reply-section">
																																<form action="{{ route('patient.messages.update', ['message' => $message->id]) }}"
																																				method="POST">
																																				@method('PUT') @csrf
																																				<label for="reply">
