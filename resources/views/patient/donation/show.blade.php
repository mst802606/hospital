...
<div class="container-fluid bg-light">
    <section>
        ...
        <div class="container-fluid mt-5">
            <a href="{{ route('patient.donations.index') }}" class="btn btn-primary m-2">Back</a>
            <a href="{{ route('patient.donations.edit',['donation'=>$donation->id]) }}" class="btn btn-info m-2">Edit</a>
            <a onclick="document.getElementById('donation-delete-form').submit()" class="btn btn-danger m-2">Delete this item</a>
            <form id="donation-delete-form" action="{{ route('patient.donations.destroy',['donation'=>$donation->id]) }}" class="d-none" method="POST">
                @method("DELETE")
                @csrf
            </form>
        </div>
